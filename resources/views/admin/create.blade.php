...
<div class="w-full max-w-screen-xl mx-auto p-10 rounded-xl"
     style="background: linear-gradient(180deg, rgba(45,35,0,0.9) 0%, rgba(0,0,0,0.7) 100%);
            ...
            box-shadow: none;"> {{-- Subtle gradient background with blur, no glow --}}
    <h2 class="text-yellow-800 font-semibold text-2xl mb-6">Add New Customer</h2>
    ...
    <form action="{{ route('customers.store') }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="date" name="payout_date" id="payout_date" value="{{ old('payout_date') }}"
                class="w-full border border-yellow-800 rounded py-2 px-3 text-yellow-900 bg-yellow-50 focus:outline-none focus:ring-2 focus:ring-yellow-600" required>
            @error('payout_date')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        ...
        <div class="mb-6">
            ...
            <input type="text" name="place_of_residence" id="place_of_residence" value="{{ old('place_of_residence') }}"
                class="w-full border border-yellow-800 rounded py-2 px-3 text-yellow-900 bg-yellow-50 focus:outline-none focus:ring-2 focus:ring-yellow-600" required>
