...
<div class="w-full max-w-screen-xl mx-auto p-10 rounded-xl"
     style="background: linear-gradient(180deg, rgba(45,35,0,0.9) 0%, rgba(0,0,0,0.7) 100%);
            ...
            box-shadow: none;">
    <h2 class="text-yellow-800 font-semibold text-2xl mb-6">Edit Transaction</h2>

    <form action="{{ route('admin.history.update', $transaction) }}" method="POST">
        @csrf
        @method('PUT')

        <div class="mb-4">
            <label for="customer_id" class="block text-yellow-800 text-sm font-semibold mb-2">Customer ID</label>
            <input type="text" name="customer_id" id="customer_id" value="{{ old('customer_id', $transaction->customer_id) }}"
        ...
        <div class="mb-4">
            ...
            <input type="text" name="from" id="from" value="{{ old('from', $transaction->from) }}"
                class="w-full border border-yellow-800 rounded py-2 px-3 text-yellow-900 bg-yellow-50 focus:outline-none focus:ring-2 focus:ring-yellow-600" required>
            @error('from')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        ...
        <div class="mb-6">
            ...
            <input type="text" name="to" id="to" value="{{ old('to', $transaction->to) }}"
                class="w-full border border-yellow-800 rounded py-2 px-3 text-yellow-900 bg-yellow-50 focus:outline-none focus:ring-2 focus:ring-yellow-600" required>
            @error('to')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
